...
def tail_drop_simulation():
	...
	return serving_tail_drop_nodes(FEL0, FEL1, FEL0_size+FEL1_size, curr_time)
...
for i in range(3):
	start = time.clock()
	output.append(list(tail_drop_simulation()))
	end = time.clock()
	time_taken.append(end - start)

output = zip(*output)
print "output: avg q1, avg q2, avg drops, avg served: "
print [np.mean(i) for i in output]
