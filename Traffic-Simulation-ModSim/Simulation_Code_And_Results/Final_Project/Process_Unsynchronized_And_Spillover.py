...
def queueing(queue, vehicles, queue_copy, next_queue, last_queue, segment_num, segment_length_list):
	...
	n = queue.qsize()
	if (not last_queue):
		max_vehicles = segment_length_list[segment_num] / vehicle_length_mu

	vehicle_counts_to_handle_spill_over = 0
	...
	if (not last_queue):
		while not queue.empty():
			...
			if (is_vehicle_first_in_queue):
				...
				temp_time = temp[0]
				vehicle_counts_to_handle_spill_over += 1
			else:
				if ( (cur_temp_time - temp_time) <= cycle_timing_list[0] and vehicle_counts_to_handle_spill_over <= max_vehicles):
					...
					vehicle_number_in_queue += 1
					vehicle_counts_to_handle_spill_over += 1
				elif ((vehicle_counts_to_handle_spill_over > max_vehicles and not (cur_temp_time - temp_time) <= cycle_timing_list[0]) ):
					if ((temp[0] % total_cyle_length) <= cycle_timing_list[0] ):
						travel_time = segment_length_list[segment_num] / float(velocity) + vehicle_number_in_queue* ((vehicle_length + offset_between_vehicles) / float(velocity)) + cycle_timing_list[1]
						next_queue.put((temp[0]+ travel_time, temp[1], temp[2]))
					else:
						travel_time = (cycle_timing_list[1] - (temp[0] % total_cyle_length - cycle_timing_list[0])) + (segment_length_list[segment_num] / float(velocity)) + vehicle_number_in_queue* ((vehicle_length + offset_between_vehicles) / float(velocity)) + cycle_timing_list[1]
						next_queue.put((temp[0]+ travel_time, temp[1], temp[2]))
					vehicle_number_in_queue += 1
					vehicle_counts_to_handle_spill_over += 1
...
def processBasedSimulation(simulation_num):
	...
	path = "./output/"
	path2 = "./unsynchronized_spillover/"
	in_file = "input.csv"
	out_file = "output.csv"
...
def vehicleArrivalActivity(lambda1, lambda2):
	print 1
